...
**Deshalb weiterhin nicht abschließend verifiziert:** ob `vault_setup` bei einem **regulär** (nicht durch das Automatisierungstool, sondern per normalem Doppelklick auf die Startmenü-Verknüpfung) gestarteten Prozess tatsächlich funktioniert — die Vermutung ist, dass es das tut (da die Pfadauflösung nachweislich korrekt ist und die Sandbox-Erklärung alle Beobachtungen konsistent erklärt), aber das wurde nicht durch einen echten, unsandboxed Klick bestätigt. Ebenso nicht gemessen: die tatsächliche Argon2id-Laufzeit auf echter Hardware, sowie der komplette Entsperren-/Wiederherstellungscode-/Crash-Pfad. **Empfehlung:** die App einmal ganz regulär über die Startmenü-Verknüpfung starten (ohne Automatisierungstool) und bestätigen, dass der Passwort-Screen ohne den obigen Fehler durchläuft.
