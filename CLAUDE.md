...
Schießsportverwaltung ("shooting sport administration") is a JavaFX desktop application for managing league results (Rundenwettkampf) for the Schützenkreis Meppen shooting association. It is a legacy, unmaintained project (see [README.md](README.md)) — the original author has stopped development, and Java 8 is a hard requirement (JavaFX 8 / `org.eclipse.fx.ide.jdt.core.JAVAFX_CONTAINER`, `PDDocument`/`COSVisitorException` from the old PDFBox 1.8 API, etc.). Do not assume newer Java or library APIs are available.

There is no build tool (no Maven/Gradle). This is an Eclipse project (`.project`/`.classpath`) using the e(fx)clipse plugin (`build.fxbuild`), with dependencies vendored as jars in [lib/](lib). To build/run, open the project in Eclipse with the e(fx)clipse plugin installed and a Java 8 JRE, or manually compile `src/**/*.java` against the jars in `lib/` plus a JavaFX 8 SDK on the classpath. There are no automated tests in the repo.

Runtime data files (created next to the executable/working directory, not checked in):
- `database.db` — local SQLite database, created/migrated automatically by [src/database/Database.java](src/database/Database.java) on first run.
- `config.properties` — read by [src/property/PropertiyFactory.java](src/property/PropertiyFactory.java); must define at minimum `first_season_id`, and for remote sync `db_user`, `db_password`, `db_port`, `db_host`, `db_name`.
...
- **`application`** — [Main.java](src/application/Main.java) is the JavaFX `Application` entry point; it just builds a `Stage`/`Scene` wrapping [MainWindow](src/view/MainWindow.java).
...
- **`database`** — [Database.java](src/database/Database.java) is a singleton (`Database.getInstance()`) wrapping two JDBC connections:
...
- **`pdf`** — [PDFFactory.java](src/pdf/PDFFactory.java) renders season results (dates, overall standings, per-shooter results) to PDF using the old Apache PDFBox 1.8 API (`PDPageContentStream`, `COSVisitorException`), driven manually with hand-computed x/y layout coordinates — no template/report engine.
- **`property`** — [PropertiyFactory.java](src/property/PropertiyFactory.java) is a tiny singleton-style loader for `config.properties` (note the misspelling of "Property" in the class/file name — this is intentional/existing, not a typo to silently "fix" without checking callers).
